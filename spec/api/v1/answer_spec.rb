describe 'Answers API', type: :request do
  ...
  let(:headers_with_params) { { 'CONTENT_TYPE' => 'application/x-www-form-urlencoded', 'ACCEPT' => 'application/json' } }
  ...
  describe 'GET /api/v1/answers/:id' do
    ...
    context 'when authorized' do
      ...
      it 'returns all public fields' do
        %w[id body user_id created_at updated_at links files_url comments].each do |attr|
  ...
  describe 'POST /api/v1/answers/' do
    ...
    it_behaves_like 'API Authorizable' do
      let(:method) { :post }
      let(:headers) { { 'CONTENT_TYPE' => 'application/x-www-form-urlencoded', 'ACCEPT' => 'application/json' } }
    end
  ...
  describe 'PATCH /api/v1/answers/' do
    ...
    context 'when authorized' do
      ...
      before do
        answer.update(body: 'NewBody')
        patch api_path, params: { access_token: access_token.token, answer: { id: answer.id, body: 'NewBody' } }, headers: headers_with_params
      end
